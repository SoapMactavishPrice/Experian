trigger Lead_PaymentDetail on Lead (before update,after update, after insert) {
    ...
    if(Trigger.IsAfter ){
        ...
        for (Lead l : Trigger.new) {
            ...
            if(l.Status=='OTP Verified'){
                // if (l.Send_Payment_Link__c) {                
                //List<Lead_Payment_Detail__c> lpayList =  new List<Lead_Payment_Detail__c>();
                List<Product__c> pList= [SELECT id, Name FROM Product__c WHERE Fee_Plan_Type__c = 'Fixed'];
                if([Select id from Lead_Payment_Detail__c where Lead__c =:l.id].Size() == 0 || [Select id from Lead_Payment_Detail__c where Lead__c =:l.id].Size() == null) {
            ...
            If(l.Status=='Sent for Agreement' && !l.Sent_for_Agreement__c){
                ...
                system.debug('****'+billing_address);
                system.debug('**Trigger**'+l);
            ...
            //added on 18th Oct 3pm by Balram
