trigger LeadConverter on Lead(before insert, before update, after update) {
    ...
    if(Trigger.isAfter && Trigger.isUpdate) {
        for (Lead l: Trigger.new) {
            ...
            System.debug('------------------->l.IsConverted'+l.IsConverted+' ----------->l.ConvertedAccountId '+l.ConvertedAccountId);
            ...
            if (l.IsConverted && l.ConvertedAccountId != null){ 
                ...
                if(l.Authorised_Signatory_Same_As_Nodal_Offic__c == false){
                    ...
                    cn.Title = l.Job_Title_Nodal__c;
                    cn.FirstName = l.First_Name_Nodal__c;
                    system.debug('****'+l.Last_Name_Nodal__c);
                    cn.LastName = l.Last_Name_Nodal__c;
                    cn.Phone = l.Tel_No_Nodal__c;
                    cn.Fax = l.Fax_No_Nodal__c;
                    cn.Contact_Role__c = 'Nodal Officer';
    ...
    /*
    // Aditya - 13/09/2022 - Create one case and assign to Jony Mony with given subject
	if(Trigger.isAfter && Trigger.isUpdate) {
        User jonyMony;
        for (Lead ld: Trigger.new) {
            if (ld.IsConverted && ld.ConvertedAccountId != null) {
                jonyMony = [select Id from User limit 1];
            }
        }
        ...
        for (Lead ld: Trigger.new) {
            if (ld.IsConverted && ld.ConvertedAccountId != null) {
                ...
                cs.Subject = 'New Member Onboarded - Company to be created on EWACS / NG';
                cs.OwnerId = jonyMony.Id;
                caseList.add(cs);
            }
        }
        if(caseList.size() > 0) {
