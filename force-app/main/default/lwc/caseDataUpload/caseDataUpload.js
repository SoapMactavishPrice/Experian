...
import insertCaseUploadChecker from '@salesforce/apex/updateCaseController.insertCaseUploadChecker';
import {
    ShowToastEvent
...
export default class CaseDataUpload extends LightningElement {
    ...
    get options() {
        return [
            ...
            {label: 'Send Compensation Email', value: 'Send Compensation Email' },
            {label: 'Send Compensation Reminder Email', value: 'Send Compensation Reminder Email' }

        ];
    }

    @track value = '';  // selected combobox value
    @track showSubmitForApprovalButton = false; // control submit button show/hide
    @track disabledSubmitApproval = true;   // To control the visibility of done and submit for approval button
    @track csvCaseRecords = [];
...
//     this.disabledSubmitApproval = false;
//     this.csvData = [];
...
// }
    ...
    handleFileChange(event) {
        ...
        if (file && file.type === 'text/csv') {
            ...
            reader.onload = () => {
                ...
                this.csvFileBase64 = btoa(binary);
                ...
                if(this.value === 'Resolved in favour of the Consumer'){
                    this.processResolvedCSV(cleanedCSV);
                } else {
                    this.processCSV(cleanedCSV);
                }
            };
    ...
    processResolvedCSV(cleanedCSV) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            let row = {};
            headers.forEach((header, idx) => row[header.trim()] = values[idx] ? values[idx].trim() : '');
            records.push(row);
        }

        this.csvCaseRecords = records;
        console.log('csvCaseRecords -->  ', JSON.parse(JSON.stringify(this.csvCaseRecords)));
    ...
    handleChange(event) {
        ...
        this.showSubmitForApprovalButton = (this.value === 'Resolved in favour of the Consumer');
    ...
    handleOpenModel() {
        ...
        this.disabledSubmitApproval = true;
    ...
    handleSubmitForApproval() {
        ...
        if (this.value === 'Resolved in favour of the Consumer'
            && this.csvCaseRecords
            && this.csvCaseRecords.length > 0
            && this.csvFileBase64) {

            insertCaseUploadChecker({
                fileType: this.value,
                caseRecords: this.csvCaseRecords,
                csvBase64: this.csvFileBase64,
            ...
            alert('No case data found or file not selected.');
        }
    }
...
@track isResolveInFaveor = false;
    processCSV(csv) {
        ...
        if (this.consumer_Status == 'Send Compensation Email' || this.consumer_Status == 'Send Compensation Reminder Email') {
            ///consloe.log('Headers==>',this.headerKeys);
            this.headerKeys = [...rows[0].split(',').map(header => header.trim()) , 'Final Status', 'Message'];
        }
        ...
        rows.slice(1).map((row, index) => {
            ...
            if (values[0] != undefined && values[0] != '') {
                ...
                else if (this.consumer_Status == 'Send Compensation Email') {
                    obj['isSendEmail'] = true;
                }

                else if (this.consumer_Status == 'Send Compensation Reminder Email') {
                    obj['reSend'] =true;
                }
...
async sendDataInChunks() {
    ...
    if (this.consumer_Status == 'Forward to Bank' || this.consumer_Status == 'Send Email to Consumer' || this.consumer_Status == 'Resolved in favour of the Consumer' ||
        ...
        this.consumer_Status == 'Matching in Favor' || this.consumer_Status == 'Incident Update' || this.consumer_Status == 'Send Compensation Email' || this.consumer_Status == 'Send Compensation Reminder Email') {
        this.chunkSize = 1;
    }
    ...
    downloadBeforeCsv() {
        ...
        else if (this.consumer_Status == 'Send Email Acknowledgement' || this.consumer_Status == 'Send Compensation Email' || this.consumer_Status == 'Send Compensation Reminder Email') {
                for (let i = 0; i < this.csvData.length; i++) {
                    let csvRow = this.csvData[i].CaseNumber.replace('\r', '') + ','
                        + this.csvData[i].Id.replace('\r', '') + ','
                       + this.csvData[i].Status.replace('\r', '') + ','
